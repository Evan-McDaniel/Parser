...
(define (program tokens)
  (if (car (linelist (take tokens (- (length tokens) 2))))
      (if (equal? (last (take tokens (- (length tokens) 1))) "$$")
          (cons "Correct" (list))
  ...
  (if (equal? null llist)
      ...
      (if (not (equal? (member "[EOL]" llist) #f))
          (if (car (line (take llist (index-of llist "[EOL]"))))
              ...
              (cons #f (string-append "Error on line: " (cdr (line (take llist (index-of llist "[EOL]"))))))
              )
          (line llist)
          )
      )
...
(define (line _line)
  (if (car (idx _line))
      (if (car (stmt(cdr (idx _line))))
          (if (car (line_tail (cdr (stmt(cdr (idx _line))))))
              (cons (car (line_tail (cdr (stmt(cdr (idx _line)))))) null)
