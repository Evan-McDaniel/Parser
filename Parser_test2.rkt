...
(define (linelist charList)
  (if (or (equal? null (scan (cons charList (list)))) (equal? "$$" (first (scan (cons charList (list))))))
      ...
      (if (not (equal? (member #\newline charList) #f))
          (if (equal? (first (get-line charList)) "Error")
              (cons #f (string-append (second (get-line charList)) (get-line-number charList null)))
              (if (car (line (get-line charList)))
                  (linelist (rest (member #\newline charList)))
                  (cons #f (string-append "Syntax error on line: " (get-line-number charList null)))
                  )
              )
          (if (equal? (first (scan (cons charList (list)))) "Error")
              (cons #f (string-append (second (scan (cons charList (list)))) (get-line-number charList null)))
              (if (car(line (scan (cons charList (list)))))
...
(define (stmt _stmt)
  (cond
    ...
    [else (cons #f "stmt did not start with a valid token")]
    )
  )
...
(define (idx _line)
  (if (equal? (first _line) "idx")
      (cons #t (rest _line))
      (cons #f (string-append "Syntax Error: line cannot begin with " (first _line)))
      )
  )
...
(define (scan pair)
  ...
  (if (equal? (car pair) null)
      ...
      (cond
        ...
        [else (list "Error" "Scan Error: invalid token in stream on line: ")]
        )
      )
  )
